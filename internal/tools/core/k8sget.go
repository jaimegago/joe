...
func (t *K8sGetTool) Parameters() llm.ParameterSchema {
	return llm.ParameterSchema{
		...
		Properties: map[string]llm.Property{
			...
			"namespace": {
				Type:        "string",
				Description: "Kubernetes namespace. Required when name is set: a get by name must name the namespace the resource lives in. Omitting it applies only when listing, where it means all namespaces.",
			},
			"name": {
				Type:        "string",
				Description: "Name of a specific resource to get. Providing it requires namespace. Omit to list all resources of the type.",
			},
		},
...
func (t *K8sGetTool) Execute(ctx context.Context, args map[string]any) (any, error) {
	...
	// Single resource get.
	//
	// A by-name get with an empty namespace can never succeed: the empty
	// namespace flows through to the dynamic client, which builds a
	// cluster-scoped path that has no by-name route for a namespaced type, so
	// the API answers a generic not-found that names none of this. Reject the
	// shape up front rather than spending a round trip to learn nothing.
	if name != "" {
		if namespace == "" {
			return nil, fmt.Errorf("k8s get resource failed: a get by name requires a namespace, but namespace was empty (resource %q, name %q); omit name to list %s across all namespaces and discover the resource names and namespaces that currently exist", resource, name, resource)
		}

		obj, err := t.client.K8sGetResource(ctx, sourceID, resource, namespace, name)
		if err != nil {
			if apierrors.IsNotFound(err) {
				return nil, fmt.Errorf("k8s get resource failed: %s %q not found in namespace %q; the name may be stale (pod names are generated by their controllers and change on restart) or the namespace may be wrong; omit name to list %s across all namespaces and discover the resource names and namespaces that currently exist: %w", resource, name, namespace, resource, err)
			}
