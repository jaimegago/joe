...
func TestEnqueueReview_Success(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodPost {
			t.Errorf("EnqueueReview: expected POST, got %s", r.Method)
		}
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusCreated)
		_ = json.NewEncoder(w).Encode(review.ReviewJob{
			ID:       "job-1",
			Platform: review.PlatformGitHub,
			Status:   review.JobStatusPending,
		})
	}))
	defer ts.Close()
	...
	created, err := c.EnqueueReview(context.Background(), job)
	if err != nil {
		t.Fatalf("EnqueueReview() error: %v", err)
	}
	if created.ID != "job-1" {
		t.Errorf("EnqueueReview(): unexpected ID %q", created.ID)
	}
}

func TestEnqueueReview_Error(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusInternalServerError)
		_ = json.NewEncoder(w).Encode(map[string]any{"error": "internal", "message": "queue full"})
	}))
	defer ts.Close()

	c := New(ts.URL)
	_, err := c.EnqueueReview(context.Background(), &review.ReviewJob{})
	if err == nil {
		t.Fatal("EnqueueReview(): expected error for 500 response")
	}
}
...
func TestGetReview_Success(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodGet {
			t.Errorf("GetReview: expected GET, got %s", r.Method)
		}
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusOK)
		_ = json.NewEncoder(w).Encode(review.ReviewJob{
			ID:     "job-99",
			Status: review.JobStatusDone,
		})
	}))
	defer ts.Close()

	c := New(ts.URL)
	job, err := c.GetReview(context.Background(), "job-99")
	if err != nil {
		t.Fatalf("GetReview() error: %v", err)
	}
	if job.ID != "job-99" {
		t.Errorf("GetReview(): unexpected ID %q", job.ID)
	}
	if job.Status != review.JobStatusDone {
		t.Errorf("GetReview(): unexpected status %q", job.Status)
	}
}
...
func TestGetReview_Error(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_ = json.NewEncoder(w).Encode(map[string]any{"error": "not_found", "message": "job not found"})
	}))
	defer ts.Close()

	c := New(ts.URL)
	_, err := c.GetReview(context.Background(), "missing")
	if err == nil {
		t.Fatal("GetReview(): expected error for 404 response")
	}
}
...
func TestListReviews_Success(t *testing.T) {
	...
	c := New(ts.URL)
	jobs, err := c.ListReviews(context.Background(), review.PlatformGitHub, review.JobStatusDone, 10)
	if err != nil {
		t.Fatalf("ListReviews() error: %v", err)
	}
	if len(jobs) != 2 {
		t.Fatalf("ListReviews(): got %d jobs, want 2", len(jobs))
	}
	assertContains(t, capturedURI, "platform=github")
	assertContains(t, capturedURI, "status=done")
	assertContains(t, capturedURI, "limit=10")
}

func TestListReviews_NoFilters(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusOK)
		_ = json.NewEncoder(w).Encode(map[string]any{
	...
	c := New(ts.URL)
	jobs, err := c.ListReviews(context.Background(), "", "", 0)
	if err != nil {
		t.Fatalf("ListReviews() no-filter error: %v", err)
	}
	if len(jobs) != 0 {
		t.Errorf("ListReviews(): got %d jobs, want 0", len(jobs))
	}
}

func TestListReviews_Error(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusInternalServerError)
		_ = json.NewEncoder(w).Encode(map[string]any{"error": "internal", "message": "db error"})
	}))
	defer ts.Close()

	c := New(ts.URL)
	_, err := c.ListReviews(context.Background(), "", "", 0)
	if err == nil {
		t.Fatal("ListReviews(): expected error for 500 response")
	}
}
...
func TestReviewURLConstruction(t *testing.T) {
	var paths []string
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		paths = append(paths, r.URL.Path)
		w.Header().Set("Content-Type", "application/json")
		switch {
		case r.Method == http.MethodPost && r.URL.Path == "/api/v1/reviews":
			w.WriteHeader(http.StatusCreated)
			_ = json.NewEncoder(w).Encode(review.ReviewJob{ID: "j1"})
		case r.Method == http.MethodGet && strings.Contains(r.URL.Path, "/api/v1/reviews/"):
			w.WriteHeader(http.StatusOK)
			_ = json.NewEncoder(w).Encode(review.ReviewJob{ID: "j1"})
		case r.Method == http.MethodGet && r.URL.Path == "/api/v1/reviews":
			w.WriteHeader(http.StatusOK)
			_ = json.NewEncoder(w).Encode(map[string]any{"jobs": []map[string]any{}, "count": 0})
		default:
			w.WriteHeader(http.StatusOK)
			_ = json.NewEncoder(w).Encode(map[string]any{})
		}
	...
	c := New(ts.URL)
	_, _ = c.EnqueueReview(context.Background(), &review.ReviewJob{})
	_, _ = c.GetReview(context.Background(), "job-abc")
	_, _ = c.ListReviews(context.Background(), "", "", 0)

	joined := strings.Join(paths, "\n")
	assertContains(t, joined, "/api/v1/reviews")
	assertContains(t, joined, "/api/v1/reviews/job-abc")
}
