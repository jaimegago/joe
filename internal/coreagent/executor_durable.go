...
// inner is typically *tools.Executor (which satisfies ToolExecutor).
...
func (d *DurableExecutor) Execute(ctx context.Context, name string, args map[string]any) (any, error) {
	...
	// to persist the key. Phase 1 design: pass through. Change 10 will
	// tighten this when the captain-session gate runs upstream.
