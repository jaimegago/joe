...
// refreshOCISource refreshes an OCI-compatible container registry source
...
func (r *Refresher) refreshOCISource(ctx context.Context, source *store.Source, adapter ociadapter.OCIAdapter) error {
	r.logger.Info("refreshing oci registry source", "source_id", source.ID)
	return r.refreshRegistrySource(ctx, source, "oci_registry", func() ([]string, error) {
		return adapter.ListRepositories(ctx)
	})
}

// refreshDockerHubSource refreshes a DockerHub source using the OCI adapter.
func (r *Refresher) refreshDockerHubSource(ctx context.Context, source *store.Source, adapter ociadapter.OCIAdapter) error {
	r.logger.Info("refreshing dockerhub source", "source_id", source.ID)
	return r.refreshRegistrySource(ctx, source, "dockerhub", func() ([]string, error) {
		return adapter.ListRepositories(ctx)
	})
}

// refreshArtifactorySource refreshes a JFrog Artifactory registry source.
// Lists Docker and Helm repositories and creates image_stored_in edges.
func (r *Refresher) refreshArtifactorySource(ctx context.Context, source *store.Source, adapter artifactoryadapter.ArtifactoryAdapter) error {
	r.logger.Info("refreshing artifactory source", "source_id", source.ID)
	...
	if err != nil {
		r.logger.Warn("failed to list artifactory repositories (skipping repo nodes)", "source_id", source.ID, "error", err)
	} else {
...
// refreshECRSource refreshes an AWS ECR source.
func (r *Refresher) refreshECRSource(ctx context.Context, source *store.Source, adapter ecradapter.ECRAdapter) error {
	r.logger.Info("refreshing ecr source", "source_id", source.ID)
	...
	repos, err := adapter.ListRepositories(ctx)
	if err != nil {
		r.logger.Warn("failed to list ECR repositories (skipping repo nodes)", "source_id", source.ID, "error", err)
	} else {
		for _, repo := range repos {
...
// refreshRegistrySource is the common refresh path for OCI-compatible registries.
// listRepos is a closure that fetches the repository list from the adapter.
func (r *Refresher) refreshRegistrySource(ctx context.Context, source *store.Source, tag string, listRepos func() ([]string, error)) error {
	now := time.Now()
	sourceNodeID := registryNodeID(source.ID, source.Type)
	...
	repos, err := listRepos()
	if err != nil {
		r.logger.Warn("failed to list registry repositories (skipping repo nodes)", "source_id", source.ID, "type", tag, "error", err)
	} else {
...
func (r *Refresher) applyRegistryDelta(ctx context.Context, source *store.Source, desiredNodes []graph.Node, desiredEdges []graph.Edge, tag string) error {
...
func (r *Refresher) buildImageStoredInEdges(ctx context.Context, source *store.Source, repoNodeID, repoName string, now time.Time) []graph.Edge {
...
func registryMetadata(source *store.Source) map[string]any {
