...
// dtAuthServer answers the metrics-metadata health check 200 and records the
// Authorization header (Api-Token <token>) so a test can assert which token the
// seam routed.
func dtAuthServer(t *testing.T, gotAuth *string) *httptest.Server {
	t.Helper()
...
func TestConnect_StaticProvider_ResolvesInlineValue(t *testing.T) {
	var gotAuth string
	srv := dtAuthServer(t, &gotAuth)

	a := dynatrace.New()
	cfg := fmt.Sprintf(`{"url":%q,"token":"placeholder","credential_provider":"static","value":"resolved-tok"}`, srv.URL)
	if err := a.Connect(context.Background(), store.Component{ID: "dt-1", Config: []byte(cfg)}); err != nil {
...
func TestConnect_NoDiscriminator_PreservesLegacyToken(t *testing.T) {
	...
	if err := a.Connect(context.Background(), store.Component{ID: "dt-2", Config: []byte(cfg)}); err != nil {
		t.Fatalf("Connect: %v", err)
	}
...
func TestConnect_ResolveFailure_SurfacesWithoutCredential(t *testing.T) {
	...
	err := a.Connect(context.Background(), store.Component{ID: "dt-3", Config: []byte(cfg)})
	if err == nil {
