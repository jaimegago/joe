...
// terraform is the unwired fixture. It must be a type that is BOTH registrable and
// absent from the wired-type registry, and after trim-unsupported-component-types
// that intersection is exactly {terraform, envoy} — both credential-less types,
// which is what keeps the reject-unwired guard reachable in production at all. The
// previous fixture was datadog (unwired because its credential is an api_key +
// app_key pair, which A003-W2 left out of the static-token batch); it became
// unregistrable in that trim, so registration 400s before promotion is reached.
// The guard itself does not distinguish why a type is unwired — absent from
// wiredTypes is the whole test — so the swap preserves what this pins.
func TestPromote_RejectsUnwiredType(t *testing.T) {
	...
	w := f.do(http.MethodPost, "/api/v1/components/c-tf/promote",
		`{"env_var":"TF_TOKEN"}`, "user:alice")
	if w.Code != http.StatusBadRequest {
		t.Fatalf("promote unwired terraform: status=%d body=%s; want 400", w.Code, w.Body.String())
	}
	...
	if !strings.Contains(w.Body.String(), "no credential provider wired for type terraform") {
		t.Errorf("reject-unwired error did not name the unwired type: %s", w.Body.String())
	}
	cfg := componentConfigMap(t, f, "c-tf")
