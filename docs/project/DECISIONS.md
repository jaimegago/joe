...
- Decision: The first-run correction pass obliged by `docs/RELEASING.md`'s
  ...
  [`docs/backlog/quickstart-download-first.md`](../backlog/done/quickstart-download-first.md)
...
- Date: 2026-07-14
- Session: k8s-get-thrash
- Decision: `k8s_get` (`internal/tools/core/k8sget.go`) mixed a list branch and a
  by-name branch behind one **optional** `namespace`, whose prose ("Omit or leave
  empty for all namespaces") was true **only on the list branch**. On the by-name
  branch an empty namespace flowed verbatim to the dynamic client, which builds a
  **cluster-scoped path that has no by-name route for a namespaced type** — a
  guaranteed not-found whose text named none of this. The fix is three parts, all
  **at the tool layer**: (1) **pre-flight rejection** — name set plus empty namespace
  returns an error **before any client call** (`k8sget.go:82-84`), carrying the
  recovery path (the stable phrase `omit name to list`); (2) **conditional prose** —
  the `namespace` description now states it is required when `name` is set and that
  omitting it applies only when listing (`k8sget.go:49`), the `name` description
  states that providing it requires `namespace` (`k8sget.go:53`), and the tool
  `Description` states that **pod names are generated by their controllers** and that
  listing first is how to discover current names and namespaces (`k8sget.go:32`);
  (3) **not-found enrichment** — a by-name failure is tested with
  `apierrors.IsNotFound` and, when it matches, the error names **the namespace
  actually tried**, notes the name may be stale or the namespace wrong, and repeats
  the recovery path (`k8sget.go:88-90`). Non-not-found errors keep the plain wrap.
  **The schema properties and the `Required` list are byte-identical** — no schema
  shape change — and **no classification change**.
- Basis: a live diagnostic run burned roughly half its agent-loop budget on errored
  by-name pod gets with an empty namespace, each returning a generic not-found that
  taught the model nothing, so it retried the same shape. Phase-1 recon verified the
  mechanism end to end: the unguarded branch dispatch (`k8sget.go:69-73` pre-change),
  the adapter's `Namespace(namespace).Get(...)` (`internal/adapters/k8s/resources.go:48`),
  and that `apierrors.IsNotFound` unwraps through the existing `fmt.Errorf` `%w` chain
  and the `mapAccessError` passthrough (`internal/api/inproc_client.go:682`) because
  apimachinery's `reasonAndCodeForError` uses `errors.As`
  (`k8s.io/apimachinery@v0.35.2/pkg/api/errors/errors.go:817-822`). Pinned by
  `TestK8sGetTool_ByName_EmptyNamespace_PreFlightRejects` (asserts the client is never
  called), `TestK8sGetTool_ByName_NotFound_Enriched` (a real `apierrors.NewNotFound`
  buried under a `%w` layer, proving the unwrap), `TestK8sGetTool_ByName_NonNotFoundError_NotEnriched`,
  and `TestK8sGetTool_List_EmptyNamespace_StillLists`
  (`internal/tools/core/k8sget_namespace_test.go`).
- Rejected: **a schema shape change** (splitting the tool, or making `namespace`
  conditionally required in the JSON schema) — the invalid combination is a
  cross-field constraint the schema cannot express without splitting a tool whose two
  branches are genuinely one question. Also rejected: **silent fallback from a failed
  by-name get to a list**. The governed answer to an unfulfillable call is a **loud,
  instructive failure, not a quiet rescue**: a fallback would return a different
  question's answer under the caller's original framing, and this is a contract OASIS
  exercises, so the tool must fail in a way the model can learn from and correct.
- Supersedes: nothing.
- Scope: a sweep of every sibling k8s read tool found the pattern **confined to
  `k8s_get`**. `istio_resource`, `flux_resource`, and `k8s_logs` all place `namespace`
  in `Required` and separately guard it against the empty string; `opa_violations`
  hardcodes an empty namespace for cluster-scoped Gatekeeper constraints, which is
  correct; every other CRD read tool is list-only, so its omit-for-all-namespaces
  prose is true. No sibling changed; nothing deferred.
- Status: accepted.
