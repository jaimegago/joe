...
Model recommendation: whenever you emit a Claude Code prompt, append one line outside the code block recommending model tier and thinking strength for that prompt. Rubric: Phase 1 read-only verification and mechanical writes (docs triage, backlog moves, INDEX regen) — mid-tier, no thinking. Multi-file implementation executing a settled design — top-tier, low thinking. Anything touching the governance seam, RBAC, or break-tested invariants — top-tier, extended thinking, regardless of size. Test or E2E authoring — top-tier, standard thinking. State recommendations as tiers unless exact current model names have been verified this session.
