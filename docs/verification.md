...
- **Instruction surfaces** — `CLAUDE.md`, `docs/verification.md`. A green build says nothing about whether an edit to a rule an agent session reads is a legitimate one. For these diffs the `docs` class's Verify field is "Nothing" and the gate passes on skips alone, so the human read is not a layer on top of the verification — it **is** the verification.

**`docs/backlog/**` was held here until 2026-08-18 and no longer is.** The reasoning was sound in isolation — joe-pm's trust boundary founds its second work source on joe's `main` being maintainer-merged, so reviewing a backlog edit is reviewing a future agent work order. It composed badly with joe-pm's `decisions/0002`, which routes **all** product work through `docs/backlog/`: every thread files or archives an item as part of its own work, so **13 of the 15 pull requests before the change touched a held path** and the hands-off auto-merge this guard exists to preserve fired twice in fifteen. A hold that catches everything holds nothing in particular. What is given up is real — an executor session can now write its own future work orders through a backlog edit that merges on the gate alone — and is accepted rather than absorbed. See joe-pm `decisions/0014-retract-the-review-cascade.md`.
...
**A second family used to be held here and no longer is.** `internal/rbac/**`, `internal/access/**` and `test/integration/rbac_test.go` were a broad path approximation of the `rbac` class, and that approximation is knowingly wrong in both directions — the class's real scope is any change to authorization or permission logic under `internal/`, which no path list decides. An approval on that family therefore attested to nothing. Those diffs are held instead by the machine verification they already owe and by the maintainer's approving review on the pull request, which does not auto-merge. **The governance-floor review in joe-pm that this sentence used to name is retracted** — it produced 11 non-clearing reviews in 12, almost all finding defects in its own prose, and joe-pm `decisions/0014-retract-the-review-cascade.md` carries the measurement. So the thing the removed path family stood in for is now preserved by a weaker mechanism than the one promised here, and that is stated rather than left for a reader to discover.
