...
- **former-employer decoupling + history-scrub blocker** (rewrite the 3 commits carrying `a former-employer address`, purge ~71 MB of compiled-binary blobs, `git-filter-repo` Path B) and **launch-blocker B1–B4 status** — deferred pre-publish work → **a backlog file** (e.g. `docs/backlog/history-scrub.md`). Partially preserved already in the tracked `docs/security-findings-punchlist.md` (`:91-126`, "history scrub (Stream C)") — so the scrub mechanics are not wholly lost, but the consolidated launch-blocker checklist is.
