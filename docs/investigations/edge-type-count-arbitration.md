...
- The same repo's own enumerations agree on 19 and contradict the "20" prose: `docs/joe-architecture.md:2355` lists exactly these 19 values and calls them *"all 19 types"*, and `docs/milestones-completed.md:238` says *"19 graph relation types in `internal/graph/relations.go`"* and lists the same 19.
- `CLAUDE.md:12` likewise says *"(19 edge types)"*.

So the 19↔20 gap is **off-by-one documentation drift in `JOE_PROJECT_KNOWLEDGE.md`**, not a real disagreement about a specific edge value. The investigation that "counted 19 against `relations.go`, verified inclusively" counted the file correctly. The "20" report simply repeated the stale JPK prose; the prior coherence audit had already flagged that "20" as *"unverified against code"* ([docs/coherence-audit.md:32](../coherence-audit.md)), and the JPK-migration triage flagged it as staleness to fix at the source ([docs/investigations/jpk-migration-triage.md:112](jpk-migration-triage.md)).
