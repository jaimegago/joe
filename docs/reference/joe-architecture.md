...
Relational data lives in the SQL store (`internal/store/`, default `~/.joe/joe.db`), migrated via embedded golang-migrate migrations under `internal/store/migrations/`. The store is **SQLite-only in practice today**: the migration runner has a PostgreSQL branch (`migratePostgres.WithInstance`) and the repositories are dialect-aware through the placeholder rewriter, but the embedded migration **SQL is SQLite-dialect-locked**. Six migrations (`001_initial`, `006_rbac`, `015_audit_log`, `018_auth_login`, `020_admin_audit`, `027_audit_session_lifecycle_kind`) carry SQLite-only constructs that PostgreSQL rejects — the two blockers are `INTEGER PRIMARY KEY AUTOINCREMENT` and the SQLite-specific append-only trigger DDL (`CREATE TRIGGER … BEFORE UPDATE/DELETE … BEGIN SELECT RAISE(ABORT, …); END`) that enforces the `audit_log` append-only invariant. Setting `database.driver: "pgx"` therefore fails at `Store.Migrate()` before serving; PostgreSQL is a latent, not-yet-functional backend (see [`docs/backlog/postgres-backend-completion.md`](../backlog/postgres-backend-completion.md)). The migration set grows over time — recent additions include `028_read_posture` (the install-wide read posture singleton) and `029_drop_joe_file_cache` (the `.joe/` cleanup). Representative tables:
...
**Environment overrides** (`internal/config/`, `internal/env/keys.go`): `JOE_LLM_PROVIDER` (the provider name, e.g. `claude`, `gemini`, or `openai-compat` — see `internal/llmfactory/factory.go` for the selectable set), `JOE_LLM_MODEL`, `JOE_SERVER`, `JOE_API_KEY`, `JOE_DATABASE_DSN`, `JOE_LOG_LEVEL`, and `JOE_MODE` (observation is the day-one boot default, raising the write floor when `JOE_MODE` is unset or set to `observation`; `full` is refused as not-yet-implemented, per D-0073). Provider auto-selection ranges over the native key-bearing providers only: an explicit preference wins; with both Anthropic and Gemini keys present Joe keeps Claude; with one key it switches to that provider; with none it errors naming both. `openai-compat` is never auto-selected — it is chosen explicitly via provider plus `base_url`.
...
- **Write floor (D-0018):** boot-resolved and runtime-immutable. Observation mode — the day-one boot default when `JOE_MODE` is unset (or set to `observation`; `full` is refused as not-yet-implemented, D-0073) — and a sticky safe-mode/panic state each raise it; nothing in the running binary lowers it — recovery is a restart (`internal/safety/floor.go`).
