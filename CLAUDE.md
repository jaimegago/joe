...
- Static credential env-var names are **operator-supplied, stored verbatim, and enforced unique per component** (D-0061). The locator is never computed or componentID-derived; promotion rejects an `env_var` already in use by another component. Because the component `config` blob is encrypted at rest, the guard is an application-level decrypt-and-scan at the promotion seam (`staticEnvVarConflict` in `internal/api/components.go`), not a DB UNIQUE constraint. The uniqueness scope is the whole component set (env vars are process-global), not per-type; the blast radius is the `KindStatic` subset of `wiredTypes` in `internal/credential/wiring.go` (a pointer, not a fixed count). Indirection-only is preserved — inline `value` and empty `env_var` are still refused. Pre-existing collisions are unhealable by code (forward-prevention only; no migration)
