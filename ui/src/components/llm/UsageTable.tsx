...
type Dimension = 'model' | 'principal' | 'session_id';
...
export function UsageTable({ rows, dimension, dimensionLabel }: UsageTableProps) {
  if (rows.length === 0) {
    return <EmptyState icon={Activity} title="No usage recorded" description="No LLM calls in this window yet." />;
  }
  const groups = groupByCurrency(rows);
  const header = dimension ? (dimensionLabel ?? dimensionHeader[dimension]) : undefined;
            ...
            <TableBody>
              {group.rows.map((row, i) => (
                <TableRow key={dimension ? (row[dimension] ?? i) : i}>
                  {header && <TableCell className="font-medium">{dimension && row[dimension] ? row[dimension] : '—'}</TableCell>}
