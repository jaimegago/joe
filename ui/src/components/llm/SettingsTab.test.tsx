...
const contextBudget: LLMContextBudget = {
  stored_raw: 0,
  state: 'backstop_fallback',
  effective: 0.7,
};
...
describe('SettingsTab limit labelling', () => {
  ...
  it('labels a backstop-fallback limit as default and shows the effective backstop value', () => {
    renderTab([
      { window: 'hourly', stored_raw: 0, state: 'backstop_fallback', effective: 100_000_000_000 },
    ]);
    const card = within(screen.getByTestId('limit-hourly'));
    expect(card.getByText('Default (backstop)')).toBeInTheDocument();
    // The effective value is a nano-unit threshold rendered verbatim (no
    // per-unit division, no bogus currency word) — 100e9 nano, the substituted
    // backstop, not zero.
    expect(card.getByText('100,000,000,000 nano')).toBeInTheDocument();
  });
  ...
  it('labels a configured positive limit as operator-set', () => {
    renderTab([
      {
        window: 'daily',
        stored_raw: 50_000_000_000,
        state: 'configured',
        effective: 50_000_000_000,
      },
    ]);
    const card = within(screen.getByTestId('limit-daily'));
    expect(card.getByText('Operator-set limit')).toBeInTheDocument();
    expect(card.getByText('50,000,000,000 nano')).toBeInTheDocument();
  });
...
describe('SettingsTab cost-limit write', () => {
  ...
  it('calls setCostLimit with the window and value and invalidates the settings query', async () => {
    ...
    await waitFor(() => expect(invalidate).toHaveBeenCalledWith({ queryKey: ['llm-settings'] }));
  });
