...
describe('useCurrentUser', () => {
  ...
  it('surfaces admin status from the current-user endpoint', async () => {
    mockFetch.mockResolvedValue({ principal: 'alice', is_admin: true, rbac_enabled: true, oidc_enabled: false, zones: [] });
  ...
  it('reports non-admin status', async () => {
    mockFetch.mockResolvedValue({ principal: 'bob', is_admin: false, rbac_enabled: true, oidc_enabled: false, zones: [] });
    const { Wrapper } = createWrapper();
    const { result } = renderHook(() => useCurrentUser(), { wrapper: Wrapper });
